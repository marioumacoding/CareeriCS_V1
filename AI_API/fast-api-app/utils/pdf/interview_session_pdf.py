...
def _add_cover_page(elements, report_data, title_style, body_style):
    ...
    image_path = _cover_image_path()

    elements.append(Spacer(1, 65))
    elements.append(Paragraph("Interview Session Report", title_style))
    elements.append(Spacer(1, 8))
    elements.append(Paragraph(f"<b>Name:</b> {basic.get('user_name') or 'N/A'}", body_style))
    elements.append(Paragraph(f"<b>Type:</b> {basic.get('interview_type') or 'N/A'}", body_style))
    elements.append(Paragraph(f"<b>Date:</b> {basic.get('interview_date') or 'N/A'}", body_style))
    elements.append(Spacer(1, 28))

    if image_path.exists():
        cover_image = Image(str(image_path), width=260, height=260)
        cover_image.hAlign = "CENTER"
        elements.append(cover_image)
        elements.append(Spacer(1, 16))

    elements.append(Paragraph("Generated by CareeriCS", body_style))
...
def build_session_report_pdf(db: Session, session_id: UUID, user_id: UUID | None = None):
    ...
    # --- Cover Page ---
    _add_cover_page(elements, report_data, title_style, body_style)
